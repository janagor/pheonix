...
const map<const string, const Token> SPECIAL_CHARS_AND_KEYWORDS{
    {"=", Token(ASSIGN)},    {"!", Token(BANG)},     {"+", Token(PLUS)},
    {"-", Token(MINUS)},     {"*", Token(STAR)},     {"%", Token(PERCENT)},
    {"#", Token(HASH)},      {"|", Token(PIPE)},     {"$", Token(DOLAR)},
    {";", Token(SEMICOLON)}, {",", Token(COMMA)},    {"(", Token(LPARENT)},
    {")", Token(RPARENT)},   {"{", Token(LBRACE)},   {"}", Token(RBRACE)},
    {"[", Token(LBRACKET)},  {"]", Token(RBRACKET)}, {"==", Token(EQUALS)},
    {"&&", Token(AND)},      {"||", Token(OR)},      {"<=", Token(LEQ)},
    {">=", Token(GEQ)},      {"<-", Token(LARROW)},  {"fn", Token(FN)},
    {"let", Token(LET)},     {"mut", Token(MUT)},    {"return", Token(RETURN)},
    {"if", Token(IF)},       {"else", Token(ELSE)},  {"while", Token(WHILE)},
    {"int", Token(INT)},     {"str", Token(STR)},    {"flt", Token(FLT)},
    {"bol", Token(BOL)},     {"true", Token(TRUE)},  {"false", Token(FALSE)},
};
...
const map<string, Token> MULTILINE_COMMENTS{
    ...
    {"/*" + string(COMMENT_MAX_SIZE - 4, 'a') + "*/",
     Token(ONE_LINE_COMMENT, "/*" + string(COMMENT_MAX_SIZE - 4, 'a') + "*/")},
};
...
TEST(TestLexer, testOperators) {
  string input = R"(+=*/-)";
  vector<Lexem> expected{
      {Token(PLUS), 1, 1},  {Token(ASSIGN), 1, 2}, {Token(STAR), 1, 3},
      {Token(SLASH), 1, 4}, {Token(MINUS), 1, 5},  {Token(END_OF_FILE), 1, 6},
  };
  istringstream in(input);
  Lexer l(in);
  vector<Lexem> result = lexerize(l);
...
TEST(TestLexer, testBrackets) {
  string input = R"((){}[])";
  vector<Lexem> expected{
      {Token(LPARENT), 1, 1},     {Token(RPARENT), 1, 2},
      {Token(LBRACE), 1, 3},      {Token(RBRACE), 1, 4},
      {Token(LBRACKET), 1, 5},    {Token(RBRACKET), 1, 6},
...
TEST(TestLexer, testIntegerMany) {
  ...
  vector<Lexem> expected{
      {Token(INTEGER, 1), 1, 1},   {Token(INTEGER, 2), 1, 3},
      {Token(INTEGER, 3), 1, 5},   {Token(INTEGER, 4), 1, 7},
      {Token(INTEGER, 5), 1, 9},   {Token(INTEGER, 6), 1, 11},
      {Token(INTEGER, 7), 1, 13},  {Token(INTEGER, 8), 1, 15},
      {Token(INTEGER, 9), 1, 17},  {Token(INTEGER, 10), 1, 19},
      {Token(INTEGER, 11), 1, 22}, {Token(INTEGER, 12), 1, 25},
      {Token(PLUS), 1, 27},        {Token(INTEGER, 13), 1, 28},
...
let mut b = 123<-str;
  ...
  vector<Lexem> expected{{Token(LET), 1, 1},
                         {Token(IDENTIFIER, "kaczka"), 1, 5},
                         {Token(ASSIGN), 1, 12},
                         {Token(IF), 1, 14},
                         {Token(LPARENT), 1, 17},
