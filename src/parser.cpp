...
std::unique_ptr<Node> Parser::parseVariableDeclaration() {
  ...
  return std::make_unique<VariableDeclaration>(isMutable, identifier,
                                               std::move(expression));
}
...
std::unique_ptr<Node> Parser::parseAssignementExpression() {
  ...
  return std::make_unique<AssignementExpression>(identifier,
                                                 std::move(expression));
}

std::unique_ptr<Node> Parser::parseOrExpression() {
  auto left = parseAndExpression();
  while (current.token.getTokenType() == token::OR) {
    token::TokenType op = current.token.getTokenType();
    readLex();
    auto right = parseAndExpression();
    left =
        std::make_unique<OrExpression>(std::move(left), std::move(right), op);
  }
  return left;
}
...
std::unique_ptr<Node> Parser::parseAndExpression() {
  ...
  while (current.token.getTokenType() == token::AND) {
    token::TokenType op = current.token.getTokenType();
    readLex();
    auto right = parseComparisonExpression();
    left =
        std::make_unique<AndExpression>(std::move(left), std::move(right), op);
  }
  return left;
}
std::unique_ptr<Node> Parser::parseComparisonExpression() {
  auto left = parseRelationalExpression();
  while (current.token.getTokenType() == token::EQUALS ||
         current.token.getTokenType() == token::NEQ) {
    token::TokenType op = current.token.getTokenType();
    readLex();
    auto right = parseRelationalExpression();
    left = std::make_unique<ComparisonExpression>(std::move(left),
                                                  std::move(right), op);
  }
  return left;
}
...
std::unique_ptr<Node> Parser::parseRelationalExpression() {
  ...
  while (current.token.getTokenType() == token::LESS ||
    ...
    left = std::make_unique<RelationalExpression>(std::move(left),
                                                  std::move(right), op);
  }
  return left;
}
...
std::unique_ptr<Node> Parser::parseAdditiveExpression() {
  ...
  while (current.token.getTokenType() == token::PLUS ||
         current.token.getTokenType() == token::MINUS) {
    token::TokenType op = current.token.getTokenType();
    readLex();
    auto right = parseMultiplicativeExpression();
    left = std::make_unique<AdditiveExpression>(std::move(left),
                                                std::move(right), op);
  }
  return left;
}
...
std::unique_ptr<Node> Parser::parseMultiplicativeExpression() {
  ...
  while (current.token.getTokenType() == token::STAR ||
    ...
    left = std::make_unique<MultiplicativeExpression>(std::move(left),
                                                      std::move(right), op);
  }
  return left;
}
...
std::unique_ptr<Node> Parser::parseCompositiveExpression() {
  ...
  while (current.token.getTokenType() == token::PIPE) {
    ...
    left = std::make_unique<CompositiveExpression>(std::move(left),
                                                   std::move(right));
  }
  return left;
}
...
std::unique_ptr<Node> Parser::parseCastExpression() {
  ...
  while (current.token.getTokenType() == token::LARROW) {
    readLex();
    std::unique_ptr<Node> type = parseTypeSpecifier();
    return std::make_unique<CastExpression>(std::move(expression),
                                            std::move(type));
  }
  return expression;
}
...
std::unique_ptr<Node> Parser::parsePrefixExpression() {
  ...
  if (current.token.getTokenType() == token::BANG ||
      current.token.getTokenType() == token::MINUS) {
    token::TokenType op = current.token.getTokenType();
    readLex();
    ...
    return std::make_unique<PrefixExpression>(op, std::move(expression));
  }
  if (current.token.getTokenType() == token::IDENTIFIER)
    expression = parseIdentifierLike();
  else if (current.token.getTokenType() == token::LPARENT)
...
std::unique_ptr<Node>
Parser::parseCallExpression(std::unique_ptr<Node> function) {
  std::unique_ptr<Node> result = std::move(function);
  assert(current.token.getTokenType() == token::TokenType::LPARENT);
  do {
    std::unique_ptr<Node> callArguments = parseCallArguments();
    result = std::make_unique<CallExpression>(std::move(result),
...
std::unique_ptr<Node> Parser::parseDebugExpression() {
  ...
  assert(current.token.getTokenType() == token::TokenType::LPARENT);
  std::unique_ptr<Node> callArguments = parseCallArguments();
  result = std::make_unique<DebugExpression>(std::move(result),
                                             std::move(callArguments));
...
std::unique_ptr<Node> Parser::parseLiteral() {
  ...
  } else if (current.token.getTokenType() == token::TRUE ||
