...
Integer& Integer::operator%=(int val) {
    if (val == 0) throw std::runtime_error("Modulo by zero");
    setValue(static_cast<long>(value) % static_cast<long>(val));
    return *this;
}
...
Float& Float::operator/=(double val) {
    if (val == 0.0) throw std::runtime_error("Division by zero");
    setValue(value / val);
    return *this;
}
Float& Float::operator+=(double val) {
...
Float Float::operator*(const Float& val) const {
    return (*this) * val.value;
}
Float Float::operator/(const Float& val) const {
    return (*this) / val.value;
...
void Float::setValue(double val) {
    value = val;
}
